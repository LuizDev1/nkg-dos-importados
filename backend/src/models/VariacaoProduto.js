...
async function criar(produtoId, dados) {
  const [resultado] = await pool.query(
    'INSERT INTO produto_variacoes (produto_id, nome, estoque_qtd) VALUES (?, ?, ?)',
    [produtoId, dados.nome, dados.estoque_qtd]
  );
...
async function atualizar(id, dados) {
  ...
  const [resultado] = await pool.query(
    'UPDATE produto_variacoes SET nome = ?, estoque_qtd = ?, ativo = ? WHERE id = ?',
    [dados.nome, dados.estoque_qtd, dados.ativo, id]
  );
