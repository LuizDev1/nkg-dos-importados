...
export default function SeletorFotos({ fotos, onChange, maximo = 5, rotulo = 'Fotos', disabled = false, onCarregando, limiteMb = 1, rotaUpload = 'imagens', aceitarGif = false }) {
  const id = useId();
  ...
  async function selecionar(evento) {
    ...
    const tipos = aceitarGif ? ['image/jpeg', 'image/png', 'image/webp', 'image/gif'] : ['image/jpeg', 'image/png', 'image/webp'];
    if (arquivos.some(f => !tipos.includes(f.type) || f.size > limiteMb * 1024 * 1024)) { setErro(`Use JPG, PNG, WebP${aceitarGif ? ' ou GIF' : ''} de até ${limiteMb} MB cada.`); return; }
  ...
  return <div className="col-span-2 sm:col-span-3">
    ...
    <input id={id} type="file" accept={aceitarGif ? 'image/jpeg,image/png,image/webp,image/gif' : 'image/jpeg,image/png,image/webp'} multiple={maximo > 1} disabled={disabled || carregando || fotos.length >= maximo} onChange={selecionar} className="sr-only" />
    <p className="mt-2 text-xs text-[#aaa396]">{fotos.length}/{maximo} - JPG, PNG, WebP{aceitarGif ? ' ou GIF' : ''} · até {limiteMb} MB</p>
